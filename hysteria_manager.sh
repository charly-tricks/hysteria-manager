...
DB_V1="$CONFIG_DIR/charly_users_v1.db"
DB_V2="$CONFIG_DIR/charly_users_v2.db"
...
detectar_os() {
    ...
    echo -e "${CYAN}[*] Sistema detectado: $OS $OS_VERSION${NC}"
}

# ── Dependencias multi-distro ─────────────────────────────────
instalar_dependencias() {
    ...
    case "$OS" in
        ubuntu|debian|linuxmint|kali)
            ...
            apt-get install -y -qq curl wget openssl net-tools lsof cron 2>/dev/null
            # jq
            ...
            # iptables-persistent solo en debian/ubuntu
        ...
        centos|rhel|almalinux|rocky|ol)
            ...
            yum install -y curl wget openssl net-tools lsof cronie sqlite 2>/dev/null || \
            dnf install -y curl wget openssl net-tools lsof cronie sqlite 2>/dev/null
            # jq en centos/rhel
            if ! command -v jq &>/dev/null; then
                yum install -y jq 2>/dev/null || dnf install -y jq 2>/dev/null || \
                { wget -q -O /usr/local/bin/jq https://github.com/stedolan/jq/releases/download/jq-1.6/jq-linux64 && chmod +x /usr/local/bin/jq; }
            fi
            systemctl enable crond 2>/dev/null && systemctl start crond 2>/dev/null
            ;;

        fedora)
            dnf install -y curl wget openssl net-tools lsof cronie sqlite jq 2>/dev/null
            systemctl enable crond 2>/dev/null && systemctl start crond 2>/dev/null
            ;;

        arch|manjaro)
            pacman -Sy --noconfirm curl wget openssl net-tools lsof cronie sqlite jq 2>/dev/null
            systemctl enable cronie 2>/dev/null && systemctl start cronie 2>/dev/null
            ;;

        *)
            echo -e "${YELLOW}[!] Sistema no reconocido, intentando con apt...${NC}"
            apt-get update -qq 2>/dev/null && \
            apt-get install -y -qq curl wget jq sqlite3 openssl net-tools lsof cron 2>/dev/null || \
            yum install -y curl wget jq sqlite openssl net-tools lsof cronie 2>/dev/null || \
            dnf install -y curl wget jq sqlite openssl net-tools lsof cronie 2>/dev/null
            ;;
    esac

    # Verificar que las herramientas críticas estén instaladas
    local faltantes=()
    for cmd in curl wget openssl sqlite3; do
        command -v $cmd &>/dev/null || faltantes+=($cmd)
    done

    if [ ${#faltantes[@]} -gt 0 ]; then
        echo -e "${RED}[!] No se pudieron instalar: ${faltantes[*]}${NC}"
        echo -e "${YELLOW}[!] Instalá manualmente e intentá de nuevo.${NC}"
    else
